...
Label = tk.Label(pathway, text="DEFENCE DIRECT ENTRY", font=('Arial', 25))
...
Fact = """The duration of Training is one year except for the Direct Entry Scheme, for which it is one and a half years. Graduates opting for Direct Entry (between 19-24 age group) are required to qualify Combined Defence Service Examination conducted by the UPSC and the final selection is done through the Service Selection Boards. Army Cadets from NDA and ACC join after completing three year degree course at respective training establishments. The technical graduates, who are engineering/PG candidates, are not required to write a written examination but need to be selected by respective SSBs."""
T.pack()
